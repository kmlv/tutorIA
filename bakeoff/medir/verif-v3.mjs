...
while (Date.now()-t0 < 330000) {
  ...
  if (s.paused && s.botones.length) {
    if (s.t >= 176) { P('*** price_effect — ANTES de responder ***', s); fs.writeFileSync(process.argv[4]+'-antes.json', JSON.stringify(s,null,1)); await page.screenshot({path:process.argv[4]+'-antes.png'}); break; }
...
const s0 = await read();
const pat = new RegExp(process.argv[2] || 'gira', 'i');
const gira = s0.botones.find(b=>pat.test(b)) ;
console.log('\n>>> PULSO:', gira);
await clic(gira);
await page.waitForTimeout(250); P('t+0.25s tras responder', await read()); await page.screenshot({path:process.argv[4]+'-d025.png'});
await page.waitForTimeout(1000); P('t+1.2s tras responder', await read()); await page.screenshot({path:process.argv[4]+'-d12.png'});
fs.writeFileSync(process.argv[4]+'-despues.json', JSON.stringify(await read(),null,1));
...
while (Date.now()-tf < 120000) {
  ...
  const k = `${s.estado.p1}/${s.estado.p2}/${s.estado.m}/${JSON.stringify(s.estado.fantasma)}`;
  if (k!==prev) { console.log(`[ESTADO] t=${s.t.toFixed(2)} -> ${k}`); prev=k; }
  if (s.paused && s.botones.length && s.t>180) { P('NUEVA PARADA', s); await page.screenshot({path:process.argv[4]+'-parada.png'}); break; }
  if (s.t > +(process.argv[3]||188)) { P('FIN', s); break; }
  await page.waitForTimeout(500);
}
await page.screenshot({path:process.argv[4]+'-fin.png'});
await browser.close();
console.log('LISTO');
