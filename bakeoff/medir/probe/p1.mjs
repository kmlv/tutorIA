...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    keys: Object.keys(s||{}),
    cues: (s?.media?.cues||[]).map(c=>({t:c.t, tipo:c.tipo||c.type, id:c.id})),
  };
});
console.log('duration', info.dur);
console.log('session keys', info.keys);
console.log('cues', JSON.stringify(info.cues, null, 1).slice(0, 4000));
await browser.close();
