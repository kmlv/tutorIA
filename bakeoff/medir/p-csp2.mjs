...
const diag = async(etq)=>console.log(etq, JSON.stringify(await page.evaluate((p)=>{
  const svg=document.querySelector('svg.bgraph');
  const capas=[...svg.querySelectorAll('.capa-manip')];
  const el=document.elementFromPoint(p[0],p[1]);
  return {nCapas:capas.length,
    hijosUltima: capas.length? [...capas[capas.length-1].children].map(c=>c.tagName+'/'+(c.getAttribute('class')||'')+'/r='+c.getAttribute('r')) : [],
    hijosPrimera: capas.length? [...capas[0].children].map(c=>c.tagName+'/'+(c.getAttribute('class')||'')+'/r='+c.getAttribute('r')) : [],
    ordenHijosSvg: [...svg.children].map(c=>c.tagName+'.'+(c.getAttribute('class')||'')),
    enPunto: el? el.tagName+'.'+(el.getAttribute('class')||'')+' r='+(el.getAttribute&&el.getAttribute('r')) : null};
}, S(px(33.333),py(0)))));
...
for(let i=0;i<8;i++){
  ...
  if(s.cls.includes('q-manip')){
    await diag('DIAG '+q.id+':');
    if(q.id==='q_cs_p_manip'){ 
      // probar tambien un click directo con playwright sobre el tirador
    ...
    if(q.id==='q_cs_m_manip'){await drag(S(px(33.333),py(0)),S(px(50),py(0)));await drag(S(px(0),py(100)),S(px(0),py(150)));
      console.log('  (item 1) tras drag', await page.evaluate(()=>[...document.querySelectorAll('.capa-manip .tirador')].map(t=>[+t.getAttribute('cx'),+t.getAttribute('cy')])));}
