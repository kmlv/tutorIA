...
const snap = async (tag) => {
  const s = await page.evaluate(() => {
    ...
    return {
      ...
      estado: JSON.stringify(window.__tutoria.estado()).slice(0,500),
      qPresent: !!q,
      qText: q ? q.innerText.replace(/\n+/g,' | ').slice(0,400) : null,
      dock: JSON.stringify(window.__tutoria.dock?.actual ?? null).slice(0,300),
    };
  });
  console.log(tag, JSON.stringify(s, null, 1));
};
await snap('t=84 inicial');
await page.evaluate(()=>window.__tutoria.media.play());
await page.waitForFunction('document.querySelector(".q")', null, {timeout:20000});
await snap('pregunta aparece');
await new Promise(r=>setTimeout(r,1500));
await snap('+1.5s');
await page.screenshot({path:'shot-pred1.png'});
await browser.close();
