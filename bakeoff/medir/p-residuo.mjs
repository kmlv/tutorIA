...
const graf = ()=>page.evaluate(()=>{const s=document.querySelector('svg.bgraph');
  return {textos:[...s.querySelectorAll('.capa-interceptos text')].map(t=>t.textContent), aria:s.getAttribute('aria-label'), capaManip: !!s.querySelector('.capa-manip')};});
for(let i=0;i<5;i++){
  ...
  else if(s.cls.includes('q-numeric')){ console.log('   >>> enunciado numerico:', s.enun);
