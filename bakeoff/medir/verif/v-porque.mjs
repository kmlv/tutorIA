...
const net=[]; page.on('response', async r=>{const u=r.url(); if(u.includes('/api/')){let b=null;try{b=(await r.text()).slice(0,500);}catch{} net.push({s:r.status(),u:u.replace(/.*\/api\//,''),b});}});
...
const before = await page.evaluate(()=>document.querySelectorAll('.dock-body .msg').length);
net.length=0;
await page.locator('.dock-acciones button', {hasText:'¿Por qué?'}).first().click();
for (const s of [3,6,10,15,25,40]){
  await page.waitForTimeout(s*1000 - (s===3?0:0));
  const st = await page.evaluate(()=>({n:document.querySelectorAll('.dock-body .msg').length,
    ult:Array.from(document.querySelectorAll('.dock-body .msg')).slice(-2).map(m=>m.className+' :: '+m.innerText.slice(0,100))}));
  console.log(`  t+~${s}s  msgs=${st.n} (antes ${before})`, JSON.stringify(st.ult));
  if (st.n >= before+2) break;
}
console.log('LLAMADAS API tras el click:', JSON.stringify(net.map(x=>({s:x.s,u:x.u,b:(x.b||'').slice(0,200)})),null,1));
await browser.close();
